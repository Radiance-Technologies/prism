...
class Level1Transformer(TreeTransformer):
    ...
    def transform(self, sexp: SexpNode) -> SexpNode:
        ...
        if sexp.is_list() and len(sexp) > 2 and \
            ...
            # then
            return SexpList(
                [SexpString(TreeTransformationConsts.REMOVE)] * (len(sexp) - 1)
                + [self.transform(sexp[-1])])
        # end if
        ...
        if sexp.is_list() and len(sexp) == 2 and \
            ...
            return SexpList(
                [
                    ...
                    SexpList(
                        [SexpString(TreeTransformationConsts.REMOVE)]
                        * (len(sexp[1]) - 1) + [self.transform(sexp[1][-1])]),
                ])
        # end if
                            ...
                            SexpList(
                                [
                                    SexpList(
                                        [
                                            SexpString(
                                                TreeTransformationConsts.REMOVE)
                                        ] * (len(sexp[1][0][0]) - 1)
                                        + [self.transform(sexp[1][0][0][-1])]),
                                    SexpString(TreeTransformationConsts.REMOVE),
                                ]),
                            SexpString(TreeTransformationConsts.REMOVE),
                        ]),
        ...
        if sexp.is_list() and len(sexp) >= 1 and \
            ...
            return SexpList(
                [
                    SexpString(
                        TreeTransformationConsts.UPDATE + "<" + sexp[0].content
                        + ">"),
