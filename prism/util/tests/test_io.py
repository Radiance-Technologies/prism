...
class TestAtomicWrite(unittest.TestCase):
    ...
    def test_atomic_write_str_compressed(self):
        """
        Verify compressed strings get written correctly.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            test_string = "abc\ndef"
            test_filename = Path(tmpdir) / "test.txt"
            atomic_write(
                test_filename,
                test_string,
                apply_gzip_compression=True)
            with gzip.open(str(test_filename) + ".gz", "rt") as f:
                actual_file_contents = f.read()
                self.assertEqual(test_string, actual_file_contents)
