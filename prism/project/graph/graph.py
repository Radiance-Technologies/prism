...
class ProjectGraph(nx.MultiDiGraph):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._roots: Dict[NodeId, Project] = {}
        self._root_variants: Dict[NodeId, Set[str]] = {}
    ...
    def _add_project_node(self, project_node: ProjectNode, *args, **kwargs) -> Tuple[NodeIdSet, EdgeIdSet]:
        """
    ...
    def _add_nodes(self, nodes: ProjectNode, *args, **kwargs) -> Tuple[NodeIdSet, EdgeIdSet]:
        """
    ...
    def add_dependencies(self, node_id):
        ...
        requirement nodes, and any matching libraries.
        Additionally the point will be set 
        """
        roots = self.roots
        added_nodes = set()
        added_edges = set()
        ...
        for req_node in ProjectCoqLibraryRequirement.nodes_from_graph(self):
            requirement = ProjectCoqLibraryRequirement.init_from_node(self, req_node)
            ...
            dependency = ProjectCoqDependency.from_parent(root, logical_name=requirement.requirement)
            if not self.has_node(dependency.node):
                ...
                added_edges = added_edges.union(
                    EdgeType.DependencyToRequirement.add_edge(self, dependency.node, requirement.node)
                )
            dependency_name = dependency.logical_name
            for library in self.match_dependency_to_library(dependency):
                added_edges = added_edges.union(
                    EdgeType.DependencyToLibrary.add_edge(self, dependency.node, library.node)
                )
        return added_nodes, added_edges
    ...
    def add_path(self, path: Path):
        ...
        file_node = ProjectFile.from_super(root, project_file_path=path, project_file_type=file_type)
        ...
        for coqproject_node in ProjectFileType.coqproject.find_files(self):
            ...
            for iqr_flag in iqrs:
                ...
                for iqr_path, iqr_name in iqrs[iqr_flag]:
                    iqr_node = ProjectExtractedIQR.from_parent(coqproject, Path(iqr_path), iqr_name, iqr_flag)
                    if not any(iqr_name in self._root_variants[other_id] and other_id != node_id for other_id in roots):
                        new_nodes, new_edges = self._add_node(
                            iqr_node,
                            roots,
                            True,
    ...
    def add_library_aliases(self, node_id):
        ...
        for node in ProjectExtractedIQR.nodes_from_graph(self):
            ...
            for _, library_node, alias in library_iter:
                ...
                added_nodes = added_nodes.union(new_nodes)
                added_edges = added_edges.union(new_edges)
                alias_edges = EdgeType.LibraryAliasToLibrary.add_edge(self, alias.node, library.node)
    ...
    def add_requirements(self, node_id):
        ...
        for node in ProjectCoqLibrary.nodes_from_graph(self):
            ...
            if root.node != node_id:
                continue
            project = ProjectDir(str(self.project_root), sentence_extraction_method=SentenceExtractionMethod.HEURISTIC)
            ...
            stats = parser._compute_sentence_statistics(
                sentences
            )
            for requirement in stats.requirements:
                requirement = ProjectCoqLibraryRequirement.from_parent(library, requirement=LogicalName(requirement))
                new_nodes, new_edges = self._add_node(
                            requirement,
                            roots,
                            True,
    ...
    def add_root(self, root: Project) -> Tuple[NodeIdSet, EdgeIdSet]:
        ...
        if not self.has_node(root.node):
            ...
            self._root_variants[root.node] = name_variants(root.project_path.stem)
        else:
            added_nodes, added_edges = set(), set()
        return added_nodes, added_edges
    ...
    def dump_edges(self, prefix=''):
        ...
        for edgetype in EdgeType:
            if edgetype is EdgeType.DependencyToLibrary:
                edges["DependencyToLibrary"] = self.jsonify_edges(edgetype, dep, lib)
            elif edgetype is EdgeType.DependencyToRequirement:
                edges["DependencyToRequirement"] = self.jsonify_edges(edgetype, dep, req)
    ...
    def dump_requirements(self, prefix=''):
        ...
        for node in ProjectCoqLibraryRequirement.nodes_from_graph(self):
            ...
            for dependency in NodeFinder.requirement_resolution(self, node):
                dependency = self.resolve_node_str(dependency, 'logical_name', False)
        ...
        for dep_type, dep_list in dict(local=local, cross=cross, unknown=unknown).items():
            for dependency, root, other_root in dep_list:
                ...
                if project not in projects:
                    projects[project] = {'local': [], 'cross': [], 'unknown': [], 'projects': []}
    ...
    def dump_libaries(self, prefix=''):
        ...
        for file_node in ProjectFile.nodes_from_graph(self, lambda n, d: d['filetype'] in types_):
            relative = self.nodes[file_node]['relative']
            if relative.stem in projects or any(relative.is_relative_to(p) for p in project_dirs):
    ...
    def separate_dependencies(self):
        ...
        for dependency in self.init_iter(ProjectCoqDependency):
            dependency_root = Project(dependency.context, dependency.project_path)
            source_name = dependency_root.context.revision.project_source.project_name
            found = False
            for library_node in EdgeType.DependencyToLibrary.find_target_nodes(self, dependency.node):
                found = True
                library = ProjectCoqLibrary.init_from_node(self, library_node)
    ...
    def jsonify_edges(self, edgetype, s_att, d_attr):
        ...
        for src, dst, key in edgetype.find_edges(self):
            ...
            edges.append({'source': str(src), 'destination': str(dst), 'key': str(key)})
        return edges

    def match_dependency_to_library(self, dependency):
        if dependency is None:
            raise ValueError(
                "Dependency is None, cannot find library."
            )
        ...
        libraries = ProjectCoqLibrary.nodes_from_graph(
            self,
            lambda n, d: matches_name(dependency.logical_name, d['logical_name']),
        )
        aliases = LibraryAlias.nodes_from_graph(
            self,
            lambda n, d: matches_name(dependency.logical_name, d['logical_name']),
        )
        libraries = [ProjectCoqLibrary.init_from_node(self, node) for node in libraries]
        libraries.extend(NodeFinder.find_library_from_alias(self, a) for a in aliases)
        return libraries
