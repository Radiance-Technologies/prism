...
class Project(ABC):
    ...
    def build(
        self,
        return_serapi_on_failure: bool = False,
        max_execute_loc: Optional[SexpInfo.Loc] = None,
        managed_switch_kwargs: Optional[Dict[str,
                                             Any]] = None,
        fallback_build_cwd: Optional[str] = None,
        **kwargs
    ) -> Tuple[int,
               str,
               str,
        ...
        Parameters
        ----------
        return_serapi_on_failure : bool, optional
            If True, attempt a line-by-line build if the normal build
            fails and return a SerAPI object stopped on the line that
            fails, by default False.
        managed_switch_kwargs : Optional[Dict[str, Any]], optional
            A dictionary containing keyword arguments to
            `managed_switch`.
        fallback_build_cwd : Optional[str], optional
            Working directory to use if return_serapi_on_failure is
            True. This should not be None if return_serapi_on_failure
            is True, by default None.
        max_execute_loc : Optional[SexpInfo.Loc], optional
            Location of the particular sentence in the project which
            is the maximum point of desired execution, by default None.
        max_memory: Optional[int], optional
            Max memory (bytes) allowed to make project.
        max_runtime: Optional[int], optional
            Max time (seconds) allowed to make project.
        ...
        serapi : Optional[SerAPI]
            None if return_serapi_on_failure is False.
            An interactive SerAPI session prior to the first encountered
            failure or prior to `max_execute_loc` if given.
            If `max_execute_loc` is not given and the project
            successfully builds, then None.
        fail_loc : Optional[SexpInfo.Loc]
            None if return_serapi_on_failure is False.
            Location of the failure. If no failure then the
            designated maximum execution location.
        error_msg : Optional[str]
            None if return_serapi_on_failure is False.
            Precise error message generated by the broken line.
            If no failure then None.
        ...
        ProjectBuildError
            ...
            error.
        ValueError
            If fallback_build_cwd is None and return_serapi_on_failure
            is True
        ...
        stdout = ""
        stderr = ""
        if return_serapi_on_failure and fallback_build_cwd is None:
            raise ValueError(
                "fallback_build_cwd must not be None if "
                "return_serpai_on_failure is True.")
        ...
        with self.project_logger(logger):
            try:
                ...
                if return_serapi_on_failure:
                    serapi, fail_loc, error_msg = self.build_line_by_line(
                        fallback_build_cwd, max_execute_loc)
                else:
                    raise
            else:
                if return_serapi_on_failure:
                    ...
                    if max_execute_loc is not None:
                        serapi, fail_loc, error_msg = self.step_through_file(
                            max_execute_loc.filename,
                            self.serapi_options,
                            max_execute_loc,
                            fallback_build_cwd)
        ...
        return rcode, stdout, stderr, serapi, fail_loc, error_msg

    def build_line_by_line(
        self,
        cwd: Optional[str],
        max_execute_loc: Optional[SexpInfo.Loc] = None,
        ...
        ----------
        cwd : Optional[str]
            Working directory for the line-by-line build
        max_execute_loc : Optional[SexpInfo.Loc]
        ...
        serapi : Optional[SerAPI]
            ...
            and `max_execute_loc` is None.
        fail_loc : Optional[SexpInfo.Loc]
            The location at which an error was encountered or the given
            `max_execute_loc` if no error was encountered.
        ...
        ------
        ExecutionError
            If `max_execute_loc` was given but not reached due to an
        ...
        if serapi_options is None:
            serapi_options = self.infer_serapi_options()[0]
            self.metadata.serapi_options = serapi_options
        assert serapi_options is not None, \
            "The SerAPI options must be known"
        ...
        for file in file_list:
            try:
                ...
                (serapi,
                 fail_loc,
                 error_msg) = self.step_through_file(
                     file,
                     serapi_options,
                     max_execute_loc,
                     cwd)
    ...
    def make_single_file(self,
                         cwd: PathLike,
                         filename: PathLike,
                         options: str) -> Tuple[int,
                                                str,
                                                str]:
        """
        Build the given Coq file.

        Parameters
        ----------
        cwd : PathLike
            Path to directory in which execution should take place.
        filename : PathLike
            Relative path to coq file to be built.
        options : str
            Linking options to provide to `coqc` during compilation
            process.
    ...
    def step_through_file(
        self,
        file: PathLike,
        serapi_options: SerAPIOptions,
        max_execute_loc: Optional[SexpInfo.Loc],
        cwd: Optional[str]) -> Tuple[SerAPI,
                                     Optional[SexpInfo.Loc],
                                     Optional[str]]:
        """
        ...
        ExecutionError
            ...
            error encountered on the way.
        """
        serapi = SerAPI(serapi_options, opam_switch=self.opam_switch, cwd=cwd)
        sentences = typing.cast(
            List[CoqSentence],
            self.get_sentences(
                Path(self.path) / file,
        ...
        for sentence in sentences:
            ...
            if (max_execute_loc is not None
                    and not (sentence.location < max_execute_loc)):
                break
            try:
                ...
                error_msg = ce.msg
                if (max_execute_loc is not None
                        and (max_execute_loc.filename != file
                             or fail_loc < max_execute_loc)):
