...
class Project(ABC):
    ...
    def build_interactive(
        self,
        fallback_build_cwd: str,
        max_execute_loc: Optional[SexpInfo.Loc] = None
    ) -> Tuple[Optional[SerAPI],
               Optional[SexpInfo.Loc],
               Optional[str]]:
        """
        Perform an interactive build using SerAPI, usually.

        If a normal build is successful, and max_execute_loc is None,
        SerAPI is not invoked.
